...
<v:table>
	<jsp:attribute name="tableHeader">
		<th colspan="7"><fmt:message key="label.transfere.core.Objeto.select.header"/></th>
	</jsp:attribute>
		...
		<v:row showUpdateButton="true" updateActionName="${fn:startsWith(currentItem.descricao, 'ATUALIZA') ? '' : 'core/ObjetoPublicacao/update'}" disableDelete="${!sistemaUsuarioSelecionado.administrador && currentItem.usuario != securityContext.username}">
			<v:column>
				<table cellpadding="0" cellspacing="0" id="objetos" style="width: 100%; margin: 0">
					<tr>
						<td><v:label key="label.transfere.core.Objeto.select.descricao" /></td>
						<td width="180"><v:label key="label.transfere.core.Objeto.select.destinoTransferencia" /></td>
						<td width="100"><v:label key="label.transfere.core.Objeto.select.usuario" /></td>
						<td width="50"><v:label key="label.transfere.core.Objeto.select.data" /></td>
					</tr>
					<tr>
					...
					<tr style="background-color: #D8D9F2">
						<td colspan="4">
							...
							<table cellpadding="0" cellspacing="0" style="width: 99%; margin-left: 5px" id="itens">
								<tr>
									<td width="100"><v:label key="label.transfere.core.Objeto.select.objetoItens.tipoObjeto" /></td>
									<td><v:label key="label.transfere.core.Objeto.select.objetoItens.nomeObjeto" /></td>
									<td width="100"><v:label key="label.transfere.core.Objeto.select.objetoItens.status" /></td>
								</tr>
							<c:choose>
							<c:when test="${fn:length(currentItem.objetoItens) > 1}">
