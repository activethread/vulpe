...
<c:if test="${!onlyToSee && util:isButtonRender(pageContext, buttonName, '')}">
	<v:action validate="false" labelKey="addDetail" elementId="AddDetail-${targetConfig.name}" action="addDetail" queryString="detail=${targetConfigPropertyName}" helpKey="tabularNew" icon="add" iconClass="AddDetail" config="${util:buttonConfig(pageContext, buttonName, '')}" />
</c:if>
