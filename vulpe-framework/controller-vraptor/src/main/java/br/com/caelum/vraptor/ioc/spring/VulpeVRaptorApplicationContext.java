 ...
 * @author Fabio Kung
 */
public class VulpeVRaptorApplicationContext extends AbstractRefreshableWebApplicationContext {

	public VulpeVRaptorApplicationContext(VulpeSpringBasedContainer container, BasicConfiguration config) {
		this.container = container;
		this.config = config;
	}

	private static final Logger logger = LoggerFactory
			.getLogger(VulpeVRaptorApplicationContext.class);

	public static final String RESOURCES_LIST = "br.com.caelum.vraptor.resources.list";

	private final AnnotationBeanNameGenerator beanNameGenerator = new AnnotationBeanNameGenerator();
	private final VulpeSpringBasedContainer container;
	...
	protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) {
		if (getParent() == null
				|| getParent().getBeanNamesForType(ServletContext.class).length == 0) {
			beanFactory.registerSingleton(ServletContext.class.getName(), config
					.getServletContext());
...

		{
			...
			if (directory != null) {
				...
				scanner.setResourceLoader(new WebinfClassesPatternResolver(config
						.getWebinfClassesDirectory()));
				scanner.scan("");
			} else {
				logger
		...
		ComponentScanner scanner = new ComponentScanner(beanFactory, container);
		scanner.scan(VulpeConfigHelper.getProjectPackage());
		scanner.resetFilters(true);
		scanner.scan("org.vulpe");
		scanner.scan(VulpeConfigHelper.getProjectPackage());
		if (config.hasBasePackages()) {
			scanner = new ComponentScanner(beanFactory, container);
			logger.info("Scanning packages from WEB-INF/classes and jars: "
	...
	private void registerApplicationScopedComponentsOn(DefaultListableBeanFactory beanFactory) {
		...
		for (Class<? extends StereotypeHandler> handlerType : BaseComponents
				.getStereotypeHandlers()) {
	...
	private void registerFactory(final Class<?> type, ConfigurableListableBeanFactory beanFactory) {
		if (ComponentFactory.class.isAssignableFrom(type)) {
			beanFactory
					.registerSingleton(type.getName(), new ComponentFactoryBean(container, type));
		}
	}
	 ...
	 * @return
	 */
	private BeanDefinitionHolder applyScopeOn(BeanDefinitionRegistry registry,
			BeanDefinitionHolder definition, ScopeMetadata scopeMetadata) {
	...
	public <T> T getBean(Class<T> type) {
		Map<String, ? extends T> instances = BeanFactoryUtils.beansOfTypeIncludingAncestors(this,
				type);
		if (instances.size() == 0) {
	...
	private boolean isPrimary(BeanDefinition definition) {
		return definition instanceof AbstractBeanDefinition
				&& ((AbstractBeanDefinition) definition).isPrimary();
	}
