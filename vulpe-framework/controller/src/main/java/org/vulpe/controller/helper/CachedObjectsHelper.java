...
public final class CachedObjectsHelper {
	...
	private static final Logger LOG = Logger
			.getLogger(CachedObjectsHelper.class);
	 ...
	 * @return
	 */
	private static Set<String> loadCachedClasses(
			final ServletContext servletContext) {
		scannotation(servletContext);
		final Set<String> cachedClasses = annotationDB.getAnnotationIndex()
				.get(org.vulpe.model.annotations.CachedClass.class.getName());
	 ...
	 * @return
	 */
	private static Set<String> loadCachedEnums(
			final ServletContext servletContext) {
		scannotation(servletContext);
	...
	private static void scannotation(final ServletContext servletContext) {
		if (annotationDB == null) {
			final URL urlWebInfClasses = WarUrlFinder
					.findWebInfClassesPath(servletContext);
			final URL[] urlsWebInfLib = WarUrlFinder
					.findWebInfLibClasspaths(servletContext);
			final List<URL> urls = new ArrayList<URL>();
			for (URL url : urlsWebInfLib) {
				final String jarName = url.getFile().substring(
	...
	public static void putAnnotedObjectsInCache(
			final ServletContext servletContext) {
				...
				try {
					...
					if (VulpeBaseEntity.class.isAssignableFrom(classicClass)) {
						...
						mapCachedClass.put(clazz.getSimpleName(),
								GenericServicesHelper.getService().getList(
										clazz.newInstance()));
					}
				} catch (Exception e) {
					LOG.error(e);
		...
		VulpeCacheHelper.getInstance().put(VulpeConstants.CACHED_CLASS,
				mapCachedClass);
		...
		if (cachedEnums != null && !cachedEnums.isEmpty()) {
			...
			for (String cachedEnum : cachedEnums) {
				try {
					...
					if (classicClass.isEnum()) {
						try {
							final String enumName = classicClass
									.getSimpleName();
							final Object[] values = classicClass
									.getEnumConstants();
							...
							for (Object object : values) {
								...
								final ValueBean value = new ValueBean(object
										.toString(), classicClass.getName()
							...
							LOG.debug("Reading object: ".concat(enumName)
									.concat(" [").concat(
											valuesDescricption.toString())
									.concat("]"));
			...
			VulpeCacheHelper.getInstance().put(VulpeConstants.CACHED_ENUM,
					mapCachedEnum);
			VulpeCacheHelper.getInstance().put(
					VulpeConstants.CACHED_ENUM_ARRAY, mapCachedEnumArray);
		}
	}
	...
	public static void putObjectsInCache(final ServletContext servletContext) {
		final Class<? extends VulpeBaseEntity<?>>[] cachedClass = VulpeConfigHelper
				.get(VulpeDomains.class).cachedClass();
		final Map<String, Object> mapCachedClass = new HashMap<String, Object>();
		if (cachedClass != null) {
			...
			for (Class clazz : cachedEnum) {
				try {
					...
					for (Object object : values) {
						...
						valuesDescricption.append(object);
						final ValueBean value = new ValueBean(
								object.toString(), VulpeConstants.View.LABEL
										.concat(projectName).concat(
												VulpeConstants.View.ENUM)
										.concat(enumName).concat(".").concat(
												object.toString()));
						array.append("'");
						array.append(value.getId());
						array.append("':'");
