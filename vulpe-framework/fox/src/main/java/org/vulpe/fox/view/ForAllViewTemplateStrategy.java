...
public class ForAllViewTemplateStrategy extends VulpeForAllTemplateStrategy {
	...
	public boolean preProcess(final TemplateBlock block,
		...
		if (super.preProcess(block, output, model)
			...
			if (codeGenerator == null
					|| codeGenerator.view().viewType()[0].equals(ViewType.NONE)) {
				return false;
			}
	...
	protected String getLabel(final DecoratedView view, final String name,
			final String type) {
		// label.${view.projectName}.${view.moduleName}.${field.name}.select.${field.name}
		final String classSimpleName = this.getClass().getSimpleName();
		return "label.".concat(view.getProjectName()).concat(".").concat(
				view.getModuleName()).concat(".").concat(classSimpleName)
				.concat(".").concat(type).concat(".").concat(name);
	}

	protected void prepareFields(final DecoratedClassDeclaration clazz,
			final DecoratedView view) {
		...
		for (FieldDeclaration field : clazz.getFields()) {
			...
			if (detail != null) {
				...
				if (StringUtils.isNotBlank(detail.relationship().identifier())) {
					...
					decoratedViewFieldDetail.setName(detail.relationship()
							.name());
					decoratedViewFieldDetail.setAction(detail.relationship()
							.action());
					decoratedViewFieldDetail.setSize(detail.relationship()
							.size());
					decoratedViewFieldDetail.setPopupWidth(detail
							.relationship().popupWidth());
					decoratedViewFieldDetail.setIdentifier(detail
							.relationship().identifier());
					decoratedViewFieldDetail.setDescription(detail
					...
					final List<Field> listDetailField = ReflectUtil
							.getInstance().getFields(detail.clazz());
					for (Field detailField : listDetailField) {
						final VulpeColumn column = detailField
								.getAnnotation(VulpeColumn.class);
						final DecoratedViewField decoratedViewFieldDetail = generateViewField(
								null, detailField);
						if (column != null) {
							decoratedViewFieldDetail.setAlign(column.align());
						}
						if (StringUtils.isNotBlank(decoratedViewFieldDetail
								.getType())) {
							detailFields.add(decoratedViewFieldDetail);
						}
					}
			...
			final DecoratedViewField decoratedViewField = generateViewField(
					field, null);
	...
	private DecoratedViewField generateViewField(FieldDeclaration fDeclaration,
			Field field) {
		DecoratedViewField decoratedViewField = new DecoratedViewField();
		String name = "";
		final VulpeText text = fDeclaration == null ? field
				.getAnnotation(VulpeText.class) : fDeclaration
		...
		final VulpeDate date = fDeclaration == null ? field
				.getAnnotation(VulpeDate.class) : fDeclaration
		...
		final VulpeSelect select = fDeclaration == null ? field
				.getAnnotation(VulpeSelect.class) : fDeclaration
			...
			if (StringUtils.isNotBlank(checkboxlist.list())) {
				if (StringUtils.isNotBlank(checkboxlist.list())) {
					...
					if (StringUtils.isNotBlank(checkboxlist.listValue())) {
						decoratedViewField.setListValue(checkboxlist
								.listValue());
					}
				}
				if (StringUtils.isNotBlank(checkboxlist.enumeration())) {
					decoratedViewField.setEnumeration(checkboxlist
							.enumeration());
				}
			}
