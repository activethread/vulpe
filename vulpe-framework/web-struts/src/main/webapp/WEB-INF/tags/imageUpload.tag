...
<c:if test="${show eq true}">
	...
	<c:if test="${empty imageThumbWidth}">
		<c:set var="imageThumbWidth" value="70"/>
	</c:if>
	<c:if test="${empty imageThumbHeight}">
		<c:set var="imageThumbHeight" value="52"/>
	</c:if>
	
	<c:if test="${empty imageWidth}">
		<c:set var="imageWidth" value="640"/>
	</c:if>
	<c:if test="${empty imageHeight}">
		<c:set var="imageHeight" value="480"/>
	</c:if>
	...
	<c:if test="${not empty util:getProperty(pageContext, name)}">
		<c:set var="thumbName" value="${name}_thumb"/>
		<a id="${elementId}_Image" href="${util:linkImage(pageContext, name, 'image/jpeg', '', imageWidth, imageThumbWidth)}" class="lightbox"><img border="0" src="${util:linkKey(thumbName, 'image/jpeg', '')}" width="${imageThumbWidth}" height="${imageThumbHeight}" class="thumb"/></a>
	    ...
	    $(function() {
		    ...
		    <c:otherwise>
		    vulpe.util.get('${elementId}_Image').lightBox();
		    </c:otherwise>
		    </c:choose>
	    });
	    </script>
