...
public class VulpeBaseDAOJPA<ENTITY extends VulpeEntity<ID>, ID extends Serializable & Comparable<?>>
	...
	private boolean checkIfCanDelete(final ENTITY entity) {
		boolean delete = true;
		...
		if (notDeleteIf != null) {
			...
			if (delete && notDeleteIf.conditions().value().length > 0) {
				hqlNotDeleteIf = new StringBuilder("select count(obj.id) from ")
						.append(entity.getClass().getSimpleName()).append(" obj ")
						.append(notDeleteIf.conditions().queryInject()).append(" where obj.id = ")
						.append(entity.getId()).append(" and (");
				int count = 0;
				...
				final Long size = (Long) getEntityManager().createQuery(hqlNotDeleteIf.toString())
						.getSingleResult();
				if (size == 1) {
	...
	public ENTITY find(final ENTITY entity) throws VulpeApplicationException {
		...
		final ENTITY newEntity = (ENTITY) getEntityManager()
	...
	private ENTITY dereference(ENTITY entity) {
	...
	private void mountOrder(final ENTITY entity, final StringBuilder order) {
		...
		for (final Field field : fields) {
			if ((field.isAnnotationPresent(SkipAutoFilter.class)
					...
					.getModifiers())) && !field.isAnnotationPresent(QueryParameter.class)) {
				continue;
			}
			...
			if (orderBy != null) {
				...
				order.append("obj.").append(field.getName()).append(" ")
						.append(orderBy.type().name());
			}
		}
	}

	private void mountParameters(final ENTITY entity, final Map<String, Object> params,
			final String parent) {
		final List<Field> fields = VulpeReflectUtil.getFields(entity.getClass());
			...
			for (final Field field : fields) {
				...
				try {
					...
					if (VulpeEntity.class.isAssignableFrom(valueClass)) {
						...
						if (value != null) {
							if (manyToOne != null
									|| (queryParameter != null && StringUtils
											.isNotEmpty(queryParameter.value()))) {
						...
						params.put(paramName.toString(), value);
					}
				} catch (Exception e) {
					throw new VulpeSystemException(e);
				}
	...
	protected String getHQL(final ENTITY entity, final Map<String, Object> params) {
		...
		if (namedQuery == null) {
			...
			if (replace && StringUtils.isNotEmpty(queryConfiguration.replace().select())) {
				...
				if (StringUtils.isNotEmpty(entity.getAutocomplete())) {
					...
					final List<Field> autocompleteFields = VulpeReflectUtil
					...
					if (name.contains(".")) {
						...
						if (joins.length > 0) {
							...
							for (final String join : joins) {
								...
								joinString.append(parent).append(".").append(join).append(" ")
										.append(join).append(" ");
		...
		if (replace && StringUtils.isNotEmpty(queryConfiguration.replace().where())) {
			...
			if (!params.isEmpty()) {
				...
				for (final String name : params.keySet()) {
					...
					if (queryParameter == null
							|| (StringUtils.isEmpty(queryParameter.equals().name()) && StringUtils
									.isEmpty(queryParameter.value()))) {
						String hqlAttributeName = name.startsWith("_") ? name.replace("_", "")
								: "obj." + name;
						...
						if (value instanceof String) {
							...
							hql.append("upper(").append(hqlAttributeName).append(") ")
									.append(useLike ? "like" : "=").append(" upper(:")
									.append(hqlParamName).append(")");
						} else {
							hql.append(hqlAttributeName).append(" = :").append(hqlParamName);
						}
					} else {
						if (queryParameter.value().contains(":this")) {
							if (queryParameter.value().contains("=")) {
								final String parts[] = value.toString().split("=");
								hql.append(parts[0].replace(":this", "obj." + name)).append(
										"= :" + name);
								params.put(name, parts[1]);
							} else {
								hql.append(queryParameter.value().replace(":this", "obj." + name))
										.append("= :" + name);
							}
						} else {
		...
		if (replace && StringUtils.isNotEmpty(queryConfiguration.replace().orderBy())) {
			...
			if (StringUtils.isNotEmpty(order.toString())
					&& (entity.getOrderBy().equals("obj.id") || StringUtils.isBlank(entity
							.getOrderBy()))) {
	...
	private String addHQLQueryParameter(final Parameter parameter, final String paramName,
			final Like like) {
	...
	public boolean exists(final ENTITY entity) throws VulpeApplicationException {
		...
		if (entity.getId() != null) {
			...
			hql.append(unproxy(entity).getClass().getSimpleName()).append(" obj where ");
	...
	public boolean notExistEquals(final ENTITY entity) throws VulpeApplicationException {
		...
		if (notExistEqual != null) {
			...
			for (final QueryParameter queryParameter : queryParameters) {
				...
				final String name = queryParameter.equals().name();
				if (StringUtils.isNotEmpty(queryParameter.equals().expression())) {
					hql.append(queryParameter.equals().expression());
				} else {
					hql.append(queryParameter.equals().alias()).append(".").append(name);
				}
				if (queryParameter.equals().upper()) {
					hql.append(")");
				}
				...
				if (StringUtils.isNotEmpty(queryParameter.equals().value())) {
					hql.append(queryParameter.equals().value());
				} else {
				...
				try {
					...
					if (queryParameter.type().equals(TypeParameter.DATE)) {
						value = new String(
								new SimpleDateFormat(queryParameter.pattern()).format(value));
					}
	...
	private QueryConfiguration getQueryConfiguration(final ENTITY entity) {
