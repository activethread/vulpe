...
<p><c:set var="style" value="display: inline;" /> <c:if
	test="${clearShow == false}">
	<c:set var="style" value="display: none;" />
</c:if> <v:action style="${style}" labelKey="vulpe.label.clear"
	...
	heightIcon="${heightIcon}" /> <c:set var="style"
	value="display: none;" /> <c:if test="${prepareShow}">
	<c:set var="style" value="display: inline;" />
</c:if> <v:action validate="false" layer="${(popup ? popupKey : '')}"
	style="${style}" labelKey="vulpe.label.clear"
	elementId="button_prepare_${actionConfig.formName}"
	action="${actionConfig.primitiveActionName}/prepare/ajax"
	helpKey="vulpe.help.clear"
	icon="themes/${vulpeTheme}/images/icons/button-clear-${widthIcon}x${heightIcon}.png"
	showButtonAsImage="${vulpeShowButtonAsImage}"
	showButtonIcon="${vulpeShowButtonIcon}"
	showButtonText="${vulpeShowButtonText}" widthIcon="${widthIcon}"
	heightIcon="${heightIcon}" /> <c:set var="style"
	value="display: none;" /> <c:if test="${createShow}">
	<c:set var="style" value="display: inline;" />
</c:if> <v:action validate="false" layer="${(popup ? popupKey : '')}"
	style="${style}" labelKey="vulpe.label.create"
	...
	showButtonText="${vulpeShowButtonText}" widthIcon="${widthIcon}"
	heightIcon="${heightIcon}" /> <c:set var="style"
	value="display: none;" /> <c:if test="${readShow}">
	<c:set var="style" value="display: inline;" />
</c:if> <v:action style="${style}" labelKey="vulpe.label.read"
	elementId="button_read_${actionConfig.formName}"
	layer="${actionConfig.formName}_select_table"
	action="${actionConfig.primitiveActionName}/read/ajax"
	beforeJs="vulpe.view.prepareRead(%27${actionConfig.formName}%27)"
	helpKey="vulpe.help.read"
...
<c:set var="style"
	value="display: none;" /> <c:if test="${reportShow}">
	<c:set var="style" value="display: inline;" />
</c:if> <v:action style="${style}" labelKey="vulpe.label.report"
	elementId="button_read_${actionConfig.formName}"
	layer="${actionConfig.formName}_select_table"
	action="${actionConfig.primitiveReportActionName}/read/ajax"
	helpKey="vulpe.help.report"
