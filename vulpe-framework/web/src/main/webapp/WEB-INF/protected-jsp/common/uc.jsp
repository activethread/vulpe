	...
	<div id="${actionConfig.formName}_uc" class="uc">
		<c:choose>
			<c:when test="${actionConfig.type == 'CRUD'}">
				<%@include file="/WEB-INF/protected-jsp/common/crud.jsp" %>
			</c:when>
			<c:when test="${actionConfig.type == 'SELECT'}">
				<%@include file="/WEB-INF/protected-jsp/common/select.jsp" %>
			</c:when>
			<c:when test="${actionConfig.type == 'TABULAR'}">
				<%@include file="/WEB-INF/protected-jsp/common/tabular.jsp" %>
			</c:when>
			...
			<c:when test="${actionConfig.type == 'FRONTEND'}">
				<%@include file="/WEB-INF/protected-jsp/common/backend.jsp" %>
			</c:when>
		</c:choose>
	</div>
</v:form>