...
var vulpe = {
	...
	config : {
		...
		dialogs : {
			alertTitle : "vulpe.dialog.alert.title",
			confirmationTitle : "vulpe.dialog.confirmation.title",
			warningTitle : "vulpe.dialog.warning.title",
			successTitle : "vulpe.dialog.success.title"
		},
		elements : new Array(),
		entity : "entity_",
		formName : "",
		hotKeys : [ [ "Clear", "Alt+Ctrl+Shift+del", "" ], [ "Create", "Ctrl+f8", "" ], [ "CreatePost", "Ctrl+f10,return", "dontFireInText,putSameOnReturnKey" ],
				[ "Clone", "Ctrl+f11", "" ], [ "Prepare", "Ctrl+backspace", "" ], [ "Read", "Ctrl+f9", "putSameOnReturnKey" ], [ "Report", "Ctrl+f12", "" ],
				[ "UpdatePost", "Ctrl+f10", "dontFireInText,putSameOnReturnKey" ], [ "TabularFilter", "Ctrl+f7", "" ], [ "TabularPost", "Ctrl+f10", "putSameOnReturnKey" ],
				[ "[addDetail]", "Alt+f8", "" ], [ "[deleteDetail]", "Alt+del", "" ], [ "[items]", "Ctrl+Shift+[numbers]", "" ], [ "[tabs]", "Alt+Shift+left,Alt+Shift+right", "" ] ],
		iPhone : {
			popupTop : 0
		},
		javascript : false,
	...
	util : {
		 ...
		 *
		 * @param options
		 *            {hotKey, command, override, dontFireInText,
		 *            returnKeyDontFireInText}
		 */
		...
		getPrefixIdByElement : function(element) {
			...
			var prefix = id.indexOf(vulpe.config.token.fieldIndex) != -1 ? id
					.substring(0, id.lastIndexOf(vulpe.config.token.fieldIndex) + vulpe.config.token.fieldIndex.length - 1) : id.substring(0, id
					.lastIndexOf(vulpe.config.token.dot));
		...
		getForm : function(options) {
			...
			return $.type(vulpe.config.formName) == "string" ? vulpe.util.get(vulpe.config.formName) : $(vulpe.config.formName);
		},
		...
		/**
		 * list: fieldNameA=valueA,fieldNameB=valueB,fieldNameC=valueC key:
		 * fieldNameA
		 */
		...
		/**
		 * list: fieldNameA=valueA,fieldNameB=valueB,fieldNameC=valueC function:
		 * command(key, value) {}
		 */
		loopMap : function(list, command, delimList, delimValue) {
			...
			for ( var i = 0; list && i < list.length; i++) {
				...
				if (element != null && element.className.indexOf(vulpe.config.css.fieldError) != -1) {
					vulpe.exception.hideFieldError({
						element : element
					});
				}
	...
	validate : {
		...
		validateRequired : function(config) {
			...
			if (typeof typeField == "undefined" && vulpe.util.isEmpty(config.field.val())) {
				isValid = false;
			} else if (typeField == 'text' || typeField == 'textarea' || typeField == 'file' || typeField == 'select-one' || typeField == 'radio' || typeField == 'checkbox'
					|| typeField == 'password') {
				var value = '';
				if (typeField == "select-one") {
					...
					isValid = config.field.prop("checked");
				} else {
		...
		validateAttributes : function(formName) {
			...
			if (fields && fields.length > 0) {
				fields.each(function(index) {
					var field = $(this);
					var idField = field.attr("id");
					if (!vulpe.validate.validateRequired({
						field : field
					})) {
	...
	view : {
		...
		validateSelectedToDelete : function(command) {
			...
			if (count > 0) {
				var message = (count == 1 ? vulpe.config.messages.deleteSelected : vulpe.config.messages.deleteSelecteds);
		...
		sortTable : function(options) {
			var value = vulpe.util.get(options.sortPropertyInfo).val();
			if (value == "obj.id") {
				value = "";
			}
			var order = "";
			if (value.indexOf(options.property + " desc") != -1) {
				order = "";
				var position = value.indexOf(options.property + " desc")
				if (position == 0) {
					value = value.replace(options.property + " desc,", "");
				} else {
					value = value.replace("," + options.property + " desc", "");
				}
				value = value.replace(options.property + " desc", "");
			} else if (value.indexOf(options.property) != -1) {
				order = "vulpeOrderDesc";
				value = value.replace(options.property, options.property + " desc");
			} else {
				order = "vulpeOrderAsc";
				if (vulpe.config.sortType == "LAST_COLUMN") {
					value = property;
				} else {
					value = (value == "") ? options.property : value + "," + options.property;
				}
			}
			vulpe.util.get(options.sortPropertyInfo).val(value);
			var propertyId = options.sortPropertyInfo + '_' + options.property.replace(".", "_");
			vulpe.util.get(propertyId).addClass(order);
			if (vulpe.config.sortType == "LAST_COLUMN") {
				vulpe.config.order = new Array();
			}
			vulpe.config.order[propertyId] = {
				property : options.sortPropertyInfo,
				value : value,
				css : order
			};
			...
			vulpe.view.request.submitAjax({
				url : url,
				layerFields : "vulpeSelectForm",
				layer : "vulpeSelectTable",
		...
		enableMarkUnmarkAll : function(name, parent) {
			...
			if (disabled) {
				vulpe.util.get("selectAll", parent).prop("disabled", disabled);
			} else {
				vulpe.util.get("selectAll", parent).prop("disabled", false);
			}
		},
		...
		/**
		 * values: fieldNameA=valueA,fieldNameB=valueB,fieldNameC=valueC
		 *
		 * popupProperties:
		...
		request : { // vulpe.view.request
			...
			registerFunctions : function(array, callback, key, layerFields) {
				...
				var newFunction = {
					...
					getKey : function() {
						return this.key + this.layerFields;
					}
				};
				var exists = false;
			 ...
			 *
			 * @param options
			 *            {page, url, formName, layerFields, layer, beforeJs,
			 *            afterJs}
			 */
			 ...
			 *
			 * @param options
			 *            {id, url, formName, layerFields, layer, beforeJs,
			 *            afterJs}
			 */
			 ...
			 *
			 * @param options
			 *            {detail, detailIndex, url, formName, layerFields,
			 *            layer, beforeJs, afterJs}
			 */
			submitDeleteDetail : function(options) {
				options.queryString = "now.detail=" + (options.detail == "entities" || options.detail.indexOf("entity.") != -1 ? options.detail : "entity." + options.detail)
						+ "&now.detailIndex=" + options.detailIndex;
				options.validate = false;
				vulpe.view.request.submitAjaxAction(options);
			},
			 ...
			 *
			 * @param options
			 *            {detail, url, formName, layerFields, layer, beforeJs,
			 *            afterJs}
			 */
			submitDeleteDetailSelected : function(options) {
				...
				if (count > 0) {
					var command = function() {
						...
						options.queryString = "now.detail="
								+ (options.detail == "entities" || options.detail.indexOf("entity.") != -1 ? options.detail : "entity." + options.detail);
						options.validate = false;
						vulpe.view.request.submitAjaxAction(options);
					}
					var message = (count == 1 ? vulpe.config.messages.deleteSelected : vulpe.config.messages.deleteSelecteds);
					...
					vulpe.view.alertDialog(vulpe.config.messages.selectRecordsToDelete);
				}
			},
			 ...
			 *
			 * @param options
			 *            {url, formName, layerFields, layer, beforeJs, afterJs}
			 */
			 ...
			 *
			 * @param options
			 *            {url, formName, layerFields, queryString, layer,
			 *            validate, beforeJs, afterJs}
			 */
			 ...
			 *
			 * @param options
			 *            {formName, layerFields, queryString, layer, validate,
			 *            beforeJs, afterJs}
			 */
			 ...
			 *
			 * @param options
			 *            {url, queryString, name, layerParent,
			 *            paramLayerParent, properties, expressions,
			 *            paramProperties, paramExpressions,
			 *            requiredParamProperties, requiredParamExpressions,
			 *            styleClass, beforeJs, afterJs, width}
			 */
			submitPopup : function(options) {
				...
				if (vulpe.util.isNotEmpty(options.afterJs)) {
					options.afterJs = escape("vulpe.view.request.onShowPopup({name: '") + escape(options.name) + escape("', afterJs: '") + options.afterJs + escape("', width: '")
							+ options.width + escape("'})");
				} else {
			 ...
			 *
			 * @param options
			 *            {queryString, layerParent, paramProperties,
			 *            paramExpressions, requiredParamProperties,
			 *            requiredParamExpressions}
			 */
			 ...
			 *
			 * @param options
			 *            {url, queryString, layer, beforeJs, afterJs,
			 *            afterCallback}
			 */
			 ...
			 *
			 * @param options
			 *            {url, autocomplete, value, identifier, description,
			 *            notFoundMessage}
			 */
					...
					options.afterJs = function() {
						...
						if (vulpe.util.get(description).val() == "") {
							...
							if (options.notFoundMessage) {
								...
								vulpe.exception.showFieldError({
									element : vulpe.util.get(description)
								});
							}
			 ...
			 *
			 * @param options
			 *            {url, uri, formName, layer, layerFields, beforeJs,
			 *            afterJs, individualLoading, validate, isFile}
			 */
			submitAjax : function(options) {
				...
				options.beforeJs = ''; // set empty do not validate on
										// submitPage
				...
				if (options.individualLoading) {
					...
					jQuery("select,input", "#" + options.layer).each(
							function(index) {
								...
								if (elementType != null && elementType != "hidden" && elementType != "button") {
									...
									if (elementLoading.length == 1) {
										...
										$(this).append(
												"&nbsp;<img id='" + elementLoadingId + "' class='vulpeImageFieldLoading' src='" + vulpe.config.contextPath + "/themes/"
														+ vulpe.config.theme + "/images/ajax/field-loader.gif' alt='" + vulpe.config.messages.loading + "' />");
				...
				// serialize form
				var queryStringForm = options.layerFields != "this" ? jQuery(":input[type!='file']", vulpe.util.get(options.layerFields, parent)).fieldSerialize() : "";
				if ((vulpe.util.isEmpty(queryStringForm) || queryStringForm.indexOf("controllerType") == -1)
						&& (options.layerFields != "this" || options.url.indexOf("/clear/") != -1) && !options.ignoreControlFields) {
					queryStringForm = (vulpe.util.isEmpty(queryStringForm) ? "" : queryStringForm + "&")
							+ jQuery(":input[type!='file']", $(vulpe.config.layers.controlFields, parent)).fieldSerialize();
				}
				...
				// sets values
				jQuery(":input[type!='file']", vulpe.util.getForm(options)).each(function(i) {
	...
	exception : {
		...
		showMessageError : function(message, complete) {
			...
			var messagesClose = "<div id=\"closeMessages\"><a href=\"javascript:void(0);\" onclick=\"$('" + messageLayer + "').slideUp('slow')\">" + vulpe.config.messages.close
					+ "</a></div>";
			if (!complete) {
